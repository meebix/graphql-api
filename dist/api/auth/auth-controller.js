 ...
 */
const signup = (req, res) => knex('users').where({ email: req.body.email }).first().then(user => {
  if (!user) {
...
}).then(({ user }) => res.status(201).json({ data: { user: { uuid: user.uuid } } })).catch(error => {
...
const confirmAccount = (req, res) => {
  ...
  return knex('users').where({ confirmed_token: confirmToken }).first().then(obj => {
    const user = obj;
  ...
  }).then(user => mailer.send(user, emailOpts.WELCOME_EMAIL)).then(({ user }) => {
 ...
 * @returns {Object} - JSON response {status, data}
 */
const login = (req, res) => knex('users').innerJoin('roles', 'users.role_id', 'roles.id').where({ email: req.body.email }).first().then(user => {
  ...
  if (!isMatch) {
    return knex('users').where({ uuid: user.uuid }).increment('login_attempts', 1).then(() => knex('users').where({ uuid: user.uuid }).first().then(updatedUser => User.checkForLockedAccount(updatedUser))).then(updatedUser => ({
      isMatch,
  ...
  if (accountLocked) {
    return mailer.send(user, emailOpts.UNLOCK_ACCOUNT_EMAIL).then(() => {
      throw new ApiError(authErrors.ACCOUNT_LOCKED);
    });
  }
  ...
  }, config.auth.jwt.secret, { expiresIn: config.auth.jwt.expireTime }, (e, token) => {
    if (e) throw e;
 ...
 * @returns {Object} - JSON response {status, data}
 */
const verifyAccount = (req, res) => knex('users').where({ email: req.body.email }).first().then(obj => {
  const user = obj;
  ...
  const query = knex.select('short_name', 'question').from('users_security_questions').innerJoin('users', 'users.id', 'users_security_questions.user_id').innerJoin('security_questions', 'security_questions.id', 'users_security_questions.question_id').where({ email: user.email });
...
const verifySecurityQuestions = (req, res) => {
  ...
  return knex.select('short_name', 'answer').from('users_security_questions').innerJoin('users', 'users.id', 'users_security_questions.user_id').innerJoin('security_questions', 'security_questions.id', 'users_security_questions.question_id').where({ uuid: req.body.uuid }).whereIn(['short_name', 'answer'], answers).then(queryResults => knex('users').where({ uuid: req.body.uuid }).first().then(user => {
    if (user && user.account_locked) throw new ApiError(authErrors.ACCOUNT_LOCKED);
    ...
    if (!isValid) {
      return knex('users').where({ uuid: user.uuid }).increment('security_question_attempts', 1).then(() => knex('users').where({ uuid: user.uuid }).first().then(updatedUser => User.checkForLockedAccount(updatedUser))).then(updatedUser => ({
        isValid,
        accountLocked: updatedUser.locked,
        user: updatedUser.user
      }));
    ...
    logger.info({ uuid: user.uuid, verificationType }, 'AUTH-CTRL.VERIFY-QUESTIONS: Creating verification code code');

    return User.update(user, {
      account_locked: true,
      [codeField]: generateToken(),
      [expiresField]: addMinutes(new Date(), config.auth.tokens.passwordReset.expireTime)
    });
  }).then(updatedUser => {
    let emailType;
  ...
  }).then(({ user }) => res.json({ data: { user: { uuid: user.uuid } } })).catch(error => {
    const err = formatError(error);
 ...
 * @returns {Object} - JSON response {status, data}
 */
const resetPassword = (req, res) => knex('users').where({ reset_password_code: req.body.resetPasswordCode }).first().then(obj => {
  const user = obj;
 ...
 * @returns {Object} - JSON response {status, data}
 */
const unlockAccount = (req, res) => knex('users').where({ unlock_account_code: req.body.unlockAccountCode }).first().then(obj => {
  const user = obj;
 ...
 * @returns {Object} - JSON response {status, data}
 */
const resendConfirmation = (req, res) => knex('users').where({ email: req.body.email }).first().then(user => {
...
}).then(() => res.json({ data: null })).catch(error => {
  const err = formatError(error);
 ...
 * @returns {Object} - JSON response {status, data}
 */
const resendUnlockAccount = (req, res) => knex('users').where({ email: req.body.email }).first().then(user => {
