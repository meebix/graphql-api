...
describe('Black Box Test: Auth', () => {
  ...
  describe('POST /api/auth/signup', () => {
    it('should create a new user', done => {
      request.post('/api/auth/signup').send({
        email: 'mike@xxxxxxxx.com',
        firstName: 'michael',
        lastName: 'jones',
        password: 'password'
      }).expect(201).end((err, response) => {
        ...
        User.knex().where({ email: 'mike@xxxxxxxx.com' }).first().then(user => {
          ...
          expect(isBefore(new Date(user.confirmed_expires), addMinutes(new Date(), 3))).to.be.true;
    ...
    it('should throw an error if user already created', done => {
      User.knex().where({ confirmed: false }).first().then(user => {
        request.post('/api/auth/signup').send({
          email: user.email,
          firstName: 'michael',
          lastName: 'jones',
  ...
  describe('POST /api/auth/confirm-account', () => {
    it('should confirm a user\'s account', done => {
      User.knex().where({ confirmed: false }).first().then(user => {
        request.post('/api/auth/confirm-account').query({ confirmToken: user.confirmed_token }).expect(200).end((err, response) => {
          const body = response.body;
          ...
          User.knex().where({ uuid: body.data.user.uuid }).first().then(selectedUser => {
    ...
    it('should throw an error if user\'s token not found', done => {
      request.post('/api/auth/confirm-account').query({ confirmToken: null }).expect(403).end((err, response) => {
        const body = response.body;
    ...
    it('should throw an error if the user\'s token has expired', done => {
      ...
      User.knex().where({ confirmed: false }).first().then(obj => {
        const user = obj;
        ...
        request.post('/api/auth/confirm-account').query({ confirmToken: user.confirmed_token }).expect(403).end((err, response) => {
          const body = response.body;
  ...
  describe('POST /api/auth/login', () => {
    ...
    it('should throw an error if user\'s password does not match', done => {
      User.knex().where({ confirmed: false }).first().then(user => {
        request.post('/api/auth/confirm-account').query({ confirmToken: user.confirmed_token }).expect(200).end(() => {
          request.post('/api/auth/login').send({
            email: user.email,
            password: 'bad_password'
          }).expect(401).end((e, res) => {
            const resBody = res.body;
    ...
    it('should set a reset password token for the user', done => {
      User.knex().where({ confirmed: false }).first().then(user => {
        request.post('/api/auth/confirm-account').query({ confirmToken: user.confirmed_token }).expect(200).end((err, response) => {
          ...
          request.post('/api/auth/forgot-password').send({
            email: user.email
          }).expect(200).end((e, res) => {
            const resBody = res.body;
            ...
            User.knex().where({ uuid: body.data.user.uuid }).first().then(selectedUser => {
              ...
              expect(isBefore(new Date(selectedUser.reset_password_expires), addMinutes(new Date(), 3))).to.be.true;
  ...
  describe('POST /api/auth/reset-password', () => {
      ...
      User.knex().where({ confirmed: false }).first().then(user => {
        request.post('/api/auth/confirm-account').query({ confirmToken: user.confirmed_token }).expect(200).end(() => {
          request.post('/api/auth/forgot-password').send({
            ...
            User.knex().where({ uuid: user.uuid }).first().then(selectedUser => {
              request.post('/api/auth/reset-password').query({ resetPasswordToken: selectedUser.reset_password_code }).send({
                password: 'password1'
                ...
                User.knex().where({ uuid: selectedUser.uuid }).first().then(updatedUser => {
    ...
    it('should throw an error if user not found', done => {
      request.post('/api/auth/reset-password').query({ resetPasswordToken: null }).expect(403).end((err, response) => {
        const body = response.body;
      ...
      User.knex().where({ confirmed: false }).first().then(user => {
        request.post('/api/auth/confirm-account').query({ confirmToken: user.confirmed_token }).expect(200).end(() => {
          request.post('/api/auth/forgot-password').send({
            ...
            User.knex().where({ uuid: user.uuid }).first().then(userObj => {
