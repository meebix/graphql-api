...
describe.skip('Black Box Test: Users', () => {
  ...
  describe('GET /api/users', () => {
    it('should return a list of users', done => {
      request.get('/api/users').expect(200).end((err, response) => {
    ...
    it('should throw an error if no users are returned', done => {
      exec('yarn migrate:undo && yarn migrate', () => {
        request.get('/api/users').expect(400).end((err, response) => {
          const body = response.body;
  ...
  describe('GET /api/users/:uuid', () => {
    it('should return the given user', done => {
      User.knex().first().then(user => {
        request.get(`/api/users/${user.uuid}`).expect(200).end((err, response) => {
          const body = response.body;
    ...
    it('should throw an error if no user is found', done => {
      request.get('/api/users/xyz').expect(400).end((err, response) => {
        const body = response.body;
  ...
  describe('PUT /api/users/:uuid', () => {
    it('should update the given user\'s details', done => {
      User.knex().first().then(user => {
        request.put(`/api/users/${user.uuid}`).send({
          firstName: 'Joe',
          lastName: 'York'
  ...
  describe('DELETE /api/users/:uuid', () => {
    it('should destroy the given user', done => {
      User.knex().first().then(user => {
        request.delete(`/api/users/${user.uuid}`).expect(200).end((err, response) => {
          const body = response.body;
