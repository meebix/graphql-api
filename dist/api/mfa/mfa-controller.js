...
const email = (req, res) => {
  ...
  return knex('users').where({ uuid }).first().then(entry => {
    if (!entry) {
    ...
    logger.info({ campaignId: options.campaignId, data }, 'MFA-CTRL.EMAIL: Email successfully sent');
...
const text = (req, res) => {
  ...
  return knex('users').where({ uuid }).first().then(entry => {
    ...
    return twilio.messages.create({
      ...
      logger.info({ data }, 'MFA-CTRL.EMAIL: Email successfully sent');
