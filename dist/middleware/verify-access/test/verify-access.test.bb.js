...
describe('Black Box Test: Verify-Access', () => {
  ...
  it.skip('should returns users if the requester has the correct access to the resource', done => {
    ...
    User.knex().where({ confirmed: false }).first().then(obj => {
      ...
      .then(updatedUser => {
        request.post('/api/auth/confirm-account').query({ confirmToken: updatedUser.confirmed_token }).expect(200).end(() => {
          request.post('/api/auth/login').send({
            ...
            request.get('/api/users').set('Authorization', `Bearer ${body.data.token}`).expect(200).end((e, res) => {
    ...
    User.knex().where({ confirmed: false }).first().then(user => {
      request.post('/api/auth/confirm-account').query({ confirmToken: user.confirmed_token }).expect(200).end(() => {
        request.post('/api/auth/login').send({
          ...
          request.get('/api/users').set('Authorization', `Bearer ${body.data.token}`).expect(403).end((e, res) => {
            const resBody = res.body;
