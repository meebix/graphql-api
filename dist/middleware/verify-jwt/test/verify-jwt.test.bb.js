...
describe('Black Box Test: Verify-Jwt', () => {
  ...
  it.skip('should returns users if Authorization header with correct JWT given', done => {
    config.auth.jwt = {
    ...
    User.knex().where({ confirmed: false }).first().then(user => {
      request.post('/api/auth/confirm-account').query({ confirmToken: user.confirmed_token }).expect(200).end(() => {
        request.post('/api/auth/login').send({
          ...
          request.get(`/api/users/${user.uuid}`).set('Authorization', `Bearer ${body.data.token}`).expect(200).end((e, res) => {
  ...
  it.skip('should throw an error if Authorization header contains a malformed JWT', done => {
    config.auth.jwt = {
      secret: process.env.JWT_SECRET,
      expireTime: '1h'
    };
    ...
    User.knex().where({ confirmed: false }).first().then(user => {
      request.post('/api/auth/confirm-account').query({ confirmToken: user.confirmed_token }).expect(200).end(() => {
        request.post('/api/auth/login').send({
          ...
          request.get(`/api/users/${user.uuid}`).expect(500).end((err, response) => {
            const body = response.body;
